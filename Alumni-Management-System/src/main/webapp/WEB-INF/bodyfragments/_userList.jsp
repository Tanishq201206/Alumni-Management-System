...
<c:url var="editUrl" value="/ctl/employee?uId=" />
...
<sf:form method="post"
	...
	<div class="card">
		<h5 class="card-header"
			style="background-color: #00061df7; color: white;">User</h5>
		<div class="card-body">
			<div class="row g-3">
				...
				<tbody>
					<c:forEach items="${list}" var="us" varStatus="user">
						<tr>
							...
							<td scope="row">${us.mobileNo}</td>
							<td scope="row">${us.role.name}</td>
							<c:if test="${sessionScope.user.role.id == 1}">
								
								<td>
								<c:if test="${us.id != 1}">
									<a href="${editUrl} ${us.id}" class="btn btn-sm btn-primary">Make Employee</a>
								</c:if>
								<c:if test="${us.id == 1}">
									---
								</c:if>
